...
export default function PredictionPanel({ prediction, selectedMarket, onMarketChange }: PredictionPanelProps) {
  ...
  return (
    <div>
      <h2 style={{ color: '#333', marginBottom: '1rem' }}>{t('prediction')}</h2>

      <div style={{ background: '#f9f9f9', padding: '1rem', borderRadius: '8px', marginBottom: '1rem' }}>
        <div style={{ fontSize: '0.85rem', color: '#666' }}>
          {t('model')}: {prediction.model_version} | {t('agreement')}: {formatProb(prediction.model_agreement)}
        </div>
        {prediction.probable_score && (
          <div style={{ marginTop: '0.5rem', fontSize: '0.85rem', color: '#333' }}>
            {t('probableScore')}: <strong>{prediction.probable_score.home} - {prediction.probable_score.away}</strong>
          </div>
        )}
      </div>
      ...
      {prediction.top_features && prediction.top_features.length > 0 && (
        <div style={{ background: '#f9f9f9', padding: '1rem', borderRadius: '8px' }}>
          <h3 style={{ margin: '0 0 0.75rem 0', color: '#333', fontSize: '1rem' }}>{t('topFeatures')}</h3>
