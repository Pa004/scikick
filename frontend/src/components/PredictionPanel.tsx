...
interface PredictionPanelProps {
  ...
  storyTabs?: boolean
  // Server context from the story bundle (MatchCenter falls back to local
  // fixtures when both are undefined, without refetching when null).
...
export default function PredictionPanel({ prediction, selectedMarket, onMarketChange, home, away, fixtures, analyst, bare = false, initialValue, displayMode, onDisplayModeChange, marketLayout = 'stack', storyTabs = false, contextHome, contextAway }: PredictionPanelProps) {
  const { t } = useLanguage()
      ...
      {storyTabs ? (
        <>
          ...
          <div role="tabpanel" id={`storypanel-${prediction.fixture_id}-context`} aria-labelledby={`storytab-${prediction.fixture_id}-context`} hidden={tab !== 'context'}>
            <MatchCenter home={home} away={away} fixtures={fixtures} bundleHome={contextHome} bundleAway={contextAway} />
          </div>
