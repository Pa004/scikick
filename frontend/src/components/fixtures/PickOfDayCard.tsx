...
export function PickOfDayCard({ pick, leagueName, onSelect }: PickOfDayCardProps) {
  ...
  return (
    <button
      ...
      aria-label={`${t('pickOfDay')}: ${pick.home} vs ${pick.away}, ${label} ${(pick.prob * 100).toFixed(1)}%`}
    ...
    >
      ...
      <span className="block font-semibold text-foreground">
        ...
        <span className="font-normal text-muted"> · {leagueName(pick.league)}</span>
      </span>
      <Badge variant="accent" className="mt-2">
        {label} {(pick.prob * 100).toFixed(1)}% ·{' '}
        {t('frequencyInTen').replace('{n}', String(formatFrequency(pick.prob)))}
      </Badge>
    </button>
  )
}
