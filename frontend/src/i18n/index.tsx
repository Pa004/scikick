/* eslint-disable react-refresh/only-export-components -- Context + hook must live together by design */
import { createContext, useContext, useEffect, useState, type ReactNode } from 'react'
import type { MarketCategoryKey } from '../components/marketCategories'

export type Locale = 'en' | 'es'

const DICTIONARIES = {
  en: {
    tagline: 'Football Probability Estimation Engine',
    allLeagues: 'All Leagues',
    premierLeague: 'Premier League',
    laLiga: 'La Liga',
    bundesliga: 'Bundesliga',
    serieA: 'Serie A',
    ligue1: 'Ligue 1',
    loading: 'Loading...',
    backendError: 'No connection to the server. Check your connection and try again.',
    fixtures: 'Fixtures',
    noFixtures: 'No fixtures available right now.',
    predicted: 'predicted',
    match: 'Match',
    goalscorer: 'Goalscorers',
    prediction: 'Prediction',
    model: 'Model',
    agreement: 'Agreement',
    probableScore: 'Probable score',
    topFeatures: 'Top Features',
    home: 'Home',
    draw: 'Draw',
    away: 'Away',
    over: 'Over',
    under: 'Under',
    stats: 'Stats',
    noStats: 'No results to calibrate yet. Check back after the matchday.',
    predictions: 'Predictions',
    accuracy: 'Accuracy',
    avgConfidence: 'Avg Confidence',
    byMarket: 'By Market',
    byLeague: 'By League',
    market: 'Market',
    total: 'Total',
    cold: 'cold',
    noMatchday: 'Need at least 5 matchdays of data.',
    noCalibration: 'Need at least 30 resolved predictions.',
    selectFixture: 'Select a fixture to see predictions.',
    loadingScorer: 'Loading scorer data...',
    noScorerData: 'No scorer data available for this fixture.',
    noScorerDataHint: 'No candidates yet — check back once minutes and lineups are recorded.',
    lineupConfirmed: 'Lineup confirmed',
    lineupProjected: 'Projected XI by minutes. No confirmed lineup yet',
    player: 'Player',
    team: 'Team',
    xg90: 'xG90',
    min: 'Min',
    prob: 'Goal prob.',
    league: 'League',
    marketResults: 'Results',
    marketGoals: 'Goals',
    marketHandicap: 'Handicap',
    marketCorners: 'Corners',
    marketCards: 'Cards',
    marketFirstHalf: '1st Half',
    marketHalfFull: 'Half/Full',
    marketCombined: 'Combined',
    noCalibrationData: 'No calibration data available.',
    noMatchdayData: 'No matchday data available.',
    predictedPct: 'Predicted',
    actualPct: 'Actual',
    perfect: 'Perfect',
    brierScore: 'Brier Score',
    accuracyPct: 'Accuracy',
    baseline: 'baseline',
    other: 'Other',
    searchFixtures: 'Search team or league...',
    clearSearch: 'Clear search',
    noSearchResults: 'No matches for this search.',
    featured: 'Featured',
    viewOdds: 'View 1X2 probabilities',
    displayProb: 'Probability',
    displayOdds: 'Odds',
    displayModeLabel: 'Display',
    oddsUp: 'Odds shortened',
    oddsDown: 'Odds lengthened',
    matchCenter: 'Match Center',
    form: 'Form',
    last5: 'Last 5',
    lastN: 'Last {n}',
    headToHead: 'Head to Head',
    h2hSummary: '{home} won {homeWins} · {draws} draws · {away} won {awayWins}',
    momentum: 'Momentum',
    recentPoints: 'Recent points',
    recentPointsHint: 'Points earned out of the maximum, last {n}',
    pointsOf: '{points} of {max} pts',
    newestFirst: 'most recent →',
    noFormData: 'No past results in loaded fixtures',
    noH2H: 'No past meetings in loaded fixtures',
    superCombo: 'Model Combo',
    superComboPartial: 'Partial: this fixture lacks some markets',
    superComboEmpty: 'Combo unavailable for this fixture',
    superComboEstimate: 'Estimate assuming independence',
    superComboHint: 'Rough guide: these markets influence each other, so treat it as a reference.',
    marketSearch: 'Search markets…',
    noMarketMatch: 'No matching markets.',
    activeMarketGroup: 'Contains the selected market',
    valueStoredSource: 'Registered odds · {source}',
    valueBestEu: 'best European odds',
    valueMultiBookNote: 'Each outcome may come from a different bookmaker.',
    valueManualSource: 'Your odds',
    valueStakeHint: 'Suggested stake: {pct} of your bankroll',
    valueKellyMethod: 'Quarter-Kelly method',
    pickOfDay: 'Pick of the Day',
    pickOfDayEmpty: 'No loaded predictions to pick from',
    scorerSearch: 'Search player or team...',
    teamAll: 'Both',
    showAll: 'Show all',
    showLess: 'Show less',
    showingOf: 'Showing {shown} of {total}',
    noScorerMatch: 'No players match these filters.',
    sortBy: 'Sort by',
    trustTitle: 'Calibration',
    trustBrierExcellent: 'Well calibrated. Predictions land close to actual outcomes.',
    trustBrierReasonable: 'Reasonably calibrated. Useful signal, expect some error.',
    trustBrierWeak: 'Weak calibration. Treat as experimental.',
    trustBrierNoData: 'Per matchday: not enough matchdays yet.',
    brierByMatchday: 'Brier by matchday',
    calibrationCurve: 'Calibration curve',
    trustCalibration: '{good} of {total} ranges within 10 pts of perfect calibration.',
    trustCalibrationPending: 'Calibration still pending. Check back after 30 resolved predictions.',
    trustSample: 'Based on {n} resolved predictions (calibration n={m}).',
    trustSampleFull: 'Based on {n} resolved predictions.',
    disclaimer: 'SciKick is an analytical instrument: probabilities for research, not betting advice.',
    language: 'Language',
    today: 'Today',
    tomorrow: 'Tomorrow',
    yesterday: 'Yesterday',
    verdictWin: '{team} win {n} in 10',
    verdictDraw: 'Draw {n} in 10',
    verdictKicker: 'Model verdict',
    frequencyInTen: '{n} in 10',
    analyst: 'Analyst',
    analystMode: 'Analyst mode: show technical details',
    valueTitle: 'Value check',
    valueHint: 'Paste your bookmaker odds to compare against the model.',
    valueGlossary: '+EV means the model estimates a higher probability than the odds imply.',
    valueCheck: 'Check value',
    valueEdge: 'Edge',
    valueIsValue: '+EV',
    valueNoValue: 'No value',
    naValue: 'Not available',
    marketMissing: 'This fixture has no such market.',
    oddsMissing: 'No odds for this outcome.',
    scorerFailed: 'Could not load scorers. Try again.',
    formLegend: 'W win · D draw · L loss',
    sortAsc: 'Ascending order',
    sortDesc: 'Descending order',
    formWin: 'Win',
    formDraw: 'Draw',
    formLoss: 'Loss',
    favorite: 'favorite',
    tableDate: 'Date',
    chartBins: '{n} bins',
    skipToContent: 'Skip to main content',
    tabsHintDisabled: 'Choose a fixture to enable the views',
    themeSwitchToLight: 'Switch to light theme',
    themeSwitchToDark: 'Switch to dark theme',
    showMore: 'Show more',
    showingMatches: 'Showing {shown} of {total} matches',
    myMatches: 'Followed',
    valueOnly: 'Value',
    showPast: 'Past',
    expandDates: 'Expand dates',
    collapseDates: 'Collapse dates',
    oneXTwoLegend: '1 = home win · X = draw · 2 = away win',
    showPastHint: 'Include past dates',
    showingPast: 'Showing past and upcoming fixtures',
    showingUpcoming: 'Showing upcoming fixtures from today',
    followTeam: 'Follow {team}',
    unfollowTeam: 'Unfollow {team}',
    followMatch: 'Follow match',
    unfollowMatch: 'Unfollow match',
    retry: 'Retry',
    loadingMatch: 'Loading match story...',
    matchError: 'Could not load this match. Check your connection and try again.',
    noFollowed: 'You follow no teams yet. Tap the star on any match to follow its teams.',
    noFollowedTitle: 'Follow teams to pin them here',
    noValueMatches: 'No value found in the loaded matches yet.',
    modelTrust: 'Model',
    close: 'Close',
    valueEmpty: 'Stored odds will appear here once loaded, or check your own above.',
    valueAutoFailed: 'Could not load stored odds. Paste your own above.',
    deepLinkMiss: 'That match is not in the current list.',
    dismiss: 'Dismiss',
    upcoming: 'Upcoming',
    noUpcoming: 'No upcoming matches for this team.',
    teamNotFound: 'No data for this team yet.',
    notFoundTitle: 'Page not found',
    notFoundText: 'That address does not exist. Back to the matches.',
    backToFeed: 'Back to matches',
    navMain: 'Sections',
    brandHome: 'SciKick home',
    taglineShort: 'Your matchday, made clear',
    moreOptions: 'More options',
    searchCommand: 'Search team or league',
    searchNoResults: 'No results. Try another name.',
    searchTeams: 'Teams',
    searchMatches: 'Matches',
    searchHint: 'Type to search teams and matches',
    recentTitle: 'Recent',
    statusUpcoming: '{n} upcoming',
    statusUpdated: 'upd. {m}m ago',
    statusUpdatedNow: 'just now',
    dataVintageNow: 'fresh data',
    dataVintageMinutes: 'data {m}m old',
    dataVintageHours: 'data {h}h old',
    dataVintageDays: 'data {d}d old',
    analystSwitch: 'Analyst',
    aboutModel: 'About the model',
    drawerHow: 'How SciKick works',
    drawerB1: 'Each team gets two numbers: how often they score, and how often they leak. Home edge is baked in.',
    drawerB2: 'From that spread we count home wins, draws, away wins. "Wins 7 in 10" is that count, in words.',
    drawerCalib: 'Calibrated model',
    languageName: 'Language',
    matchContext: 'Match context',
    matchMeta: 'Details',
    sectionOverview: 'Overview',
    sectionMarkets: 'Markets',
    sectionScorers: 'Scorers',
    tabContext: 'Context',
    tabValue: 'Value',
    dismissPick: "Dismiss today's pick",
    seeWhy: 'See why',
  },
  es: {
    tagline: 'Motor de Estimación de Probabilidades de Fútbol',
    allLeagues: 'Todas las Ligas',
    premierLeague: 'Premier League',
    laLiga: 'La Liga',
    bundesliga: 'Bundesliga',
    serieA: 'Serie A',
    ligue1: 'Ligue 1',
    loading: 'Cargando...',
    backendError: 'Sin conexión con el servidor. Revisa tu conexión e inténtalo de nuevo.',
    fixtures: 'Partidos',
    noFixtures: 'No hay partidos disponibles por ahora.',
    predicted: 'predicho',
    match: 'Partido',
    goalscorer: 'Goleadores',
    prediction: 'Predicción',
    model: 'Modelo',
    agreement: 'Acuerdo',
    probableScore: 'Marcador probable',
    topFeatures: 'Principales Características',
    home: 'Local',
    draw: 'Empate',
    away: 'Visitante',
    over: 'Más',
    under: 'Menos',
    stats: 'Estadísticas',
    noStats: 'Aún no hay resultados para calibrar. Vuelve tras la jornada.',
    predictions: 'Predicciones',
    accuracy: 'Precisión',
    avgConfidence: 'Confianza media',
    byMarket: 'Por Mercado',
    byLeague: 'Por Liga',
    market: 'Mercado',
    total: 'Total',
    cold: 'frío',
    noMatchday: 'Se necesitan al menos 5 jornadas de datos.',
    noCalibration: 'Se necesitan al menos 30 predicciones resueltas.',
    selectFixture: 'Selecciona un partido para ver predicciones.',
    loadingScorer: 'Cargando datos del goleador...',
    noScorerData: 'No hay datos de goleador para este partido.',
    noScorerDataHint: 'Aún sin candidatos — vuelve cuando haya minutos y alineaciones registrados.',
    lineupConfirmed: 'Alineación confirmada',
    lineupProjected: 'XI proyectado por minutos. Sin alineación confirmada',
    player: 'Jugador',
    team: 'Equipo',
    xg90: 'xG90',
    min: 'Min',
    prob: 'Prob. gol',
    league: 'Liga',
    marketResults: 'Resultados',
    marketGoals: 'Goles',
    marketHandicap: 'Handicap',
    marketCorners: 'Esquinas',
    marketCards: 'Tarjetas',
    marketFirstHalf: '1ª Mitad',
    marketHalfFull: 'Mitad/Final',
    marketCombined: 'Combinados',
    noCalibrationData: 'No hay datos de calibración.',
    noMatchdayData: 'No hay datos de jornada.',
    predictedPct: 'Predicho',
    actualPct: 'Real',
    perfect: 'Perfecto',
    brierScore: 'Brier Score',
    accuracyPct: 'Precisión',
    baseline: 'base',
    other: 'Otro',
    searchFixtures: 'Buscar equipo o liga...',
    clearSearch: 'Limpiar búsqueda',
    noSearchResults: 'Sin resultados para esta búsqueda.',
    featured: 'Destacados',
    viewOdds: 'Ver probabilidades 1X2',
    displayProb: 'Probabilidad',
    displayOdds: 'Cuotas',
    displayModeLabel: 'Vista',
    oddsUp: 'Cuota bajó',
    oddsDown: 'Cuota subió',
    matchCenter: 'Centro del partido',
    form: 'Forma',
    last5: 'Últimos 5',
    lastN: 'Últimos {n}',
    headToHead: 'Enfrentamientos',
    h2hSummary: '{home} ganó {homeWins} · {draws} empates · {away} ganó {awayWins}',
    momentum: 'Momento',
    recentPoints: 'Puntos recientes',
    recentPointsHint: 'Puntos logrados sobre el máximo, últimos {n}',
    pointsOf: '{points} de {max} pts',
    newestFirst: 'más reciente →',
    noFormData: 'Sin resultados previos en partidos cargados',
    noH2H: 'Sin enfrentamientos previos en partidos cargados',
    superCombo: 'Combinada del modelo',
    superComboPartial: 'Parcial: a este partido le faltan mercados',
    superComboEmpty: 'Combinada no disponible para este partido',
    superComboEstimate: 'Estimación asumiendo independencia',
    superComboHint: 'Guía aproximada: estos mercados se influyen entre sí, tómalo como referencia.',
    marketSearch: 'Buscar mercado…',
    noMarketMatch: 'Sin mercados que coincidan.',
    activeMarketGroup: 'Contiene el mercado elegido',
    valueStoredSource: 'Cuotas registradas · {source}',
    valueBestEu: 'mejores cuotas europeas',
    valueMultiBookNote: 'Cada signo puede ser de una casa distinta.',
    valueManualSource: 'Tus cuotas',
    valueStakeHint: 'Apuesta sugerida: {pct} de tu fondo',
    valueKellyMethod: 'Método Kelly fraccional (¼)',
    pickOfDay: 'Pick del día',
    pickOfDayEmpty: 'Sin predicciones cargadas para elegir pick',
    scorerSearch: 'Buscar jugador o equipo...',
    teamAll: 'Ambos',
    showAll: 'Ver todos',
    showLess: 'Ver menos',
    showingOf: 'Mostrando {shown} de {total}',
    noScorerMatch: 'Ningún jugador coincide con estos filtros.',
    sortBy: 'Ordenar por',
    trustTitle: 'Calibración',
    trustBrierExcellent: 'Bien calibrado. Las predicciones caen cerca del resultado real.',
    trustBrierReasonable: 'Calibración razonable. Señal útil, espera cierto error.',
    trustBrierWeak: 'Calibración débil. Tómalo como experimental.',
    trustBrierNoData: 'Por jornada: aún sin jornadas suficientes.',
    brierByMatchday: 'Brier por jornada',
    calibrationCurve: 'Curva de calibración',
    trustCalibration: '{good} de {total} rangos a menos de 10 pts de la calibración perfecta.',
    trustCalibrationPending: 'Calibración pendiente. Vuelve tras 30 predicciones resueltas.',
    trustSample: 'Basado en {n} predicciones resueltas (n={m} en calibración).',
    trustSampleFull: 'Basado en {n} predicciones resueltas.',
    disclaimer: 'SciKick es un instrumento analítico: probabilidades para investigación, no consejo de apuestas.',
    language: 'Idioma',
    today: 'Hoy',
    tomorrow: 'Mañana',
    yesterday: 'Ayer',
    verdictWin: '{team} gana {n} de cada 10',
    verdictDraw: 'Empate {n} de cada 10',
    verdictKicker: 'Veredicto del modelo',
    frequencyInTen: '{n} de cada 10',
    analyst: 'Analista',
    analystMode: 'Modo analista: muestra detalles técnicos',
    valueTitle: 'Chequeo de valor',
    valueHint: 'Pega las cuotas de tu casa de apuestas para comparar con el modelo.',
    valueGlossary: '+EV significa que el modelo estima más probabilidad de la que implica la cuota.',
    valueCheck: 'Calcular valor',
    valueEdge: 'Ventaja',
    valueIsValue: '+EV',
    valueNoValue: 'Sin valor',
    naValue: 'No disponible',
    marketMissing: 'Este partido no trae este mercado.',
    oddsMissing: 'Sin cuota para este signo.',
    scorerFailed: 'No se pudo cargar goleadores. Reintenta.',
    formLegend: 'V victoria · E empate · D derrota',
    sortAsc: 'Orden ascendente',
    sortDesc: 'Orden descendente',
    formWin: 'Victoria',
    formDraw: 'Empate',
    formLoss: 'Derrota',
    favorite: 'favorito',
    tableDate: 'Fecha',
    chartBins: '{n} tramos',
    skipToContent: 'Saltar al contenido principal',
    tabsHintDisabled: 'Elige un partido para activar las vistas',
    themeSwitchToLight: 'Cambiar a tema claro',
    themeSwitchToDark: 'Cambiar a tema oscuro',
    showMore: 'Mostrar más',
    showingMatches: 'Mostrando {shown} de {total} partidos',
    myMatches: 'Seguidos',
    valueOnly: 'Valor',
    showPast: 'Anteriores',
    expandDates: 'Expandir fechas',
    collapseDates: 'Contraer fechas',
    oneXTwoLegend: '1 = victoria local · X = empate · 2 = victoria visitante',
    showPastHint: 'Incluir fechas anteriores',
    showingPast: 'Mostrando partidos anteriores y próximos',
    showingUpcoming: 'Mostrando próximos partidos desde hoy',
    followTeam: 'Seguir a {team}',
    unfollowTeam: 'Dejar de seguir a {team}',
    followMatch: 'Seguir partido',
    unfollowMatch: 'Dejar de seguir partido',
    retry: 'Reintentar',
    loadingMatch: 'Cargando la historia del partido...',
    matchError: 'No se pudo cargar este partido. Revisa tu conexión e inténtalo de nuevo.',
    noFollowed: 'Aún no sigues equipos. Toca la estrella de un partido para seguir a sus equipos.',
    noFollowedTitle: 'Sigue equipos para fijarlos aquí',
    noValueMatches: 'Sin valor en los partidos cargados por ahora.',
    modelTrust: 'Modelo',
    close: 'Cerrar',
    valueEmpty: 'Las cuotas guardadas aparecerán aquí al cargar, o calcula las tuyas arriba.',
    valueAutoFailed: 'No se pudieron cargar las cuotas guardadas. Pega las tuyas arriba.',
    deepLinkMiss: 'Ese partido no está en la lista actual.',
    dismiss: 'Descartar',
    upcoming: 'Próximos',
    noUpcoming: 'Sin próximos partidos para este equipo.',
    teamNotFound: 'Aún no hay datos de este equipo.',
    notFoundTitle: 'Página no encontrada',
    notFoundText: 'Esa dirección no existe. Vuelve a los partidos.',
    backToFeed: 'Volver a partidos',
    navMain: 'Secciones',
    brandHome: 'SciKick inicio',
    taglineShort: 'Tu previa en números claros',
    moreOptions: 'Más opciones',
    searchCommand: 'Buscar equipo o liga',
    searchNoResults: 'Sin resultados. Prueba otro nombre.',
    searchTeams: 'Equipos',
    searchMatches: 'Partidos',
    searchHint: 'Escribe para buscar equipos y partidos',
    recentTitle: 'Recientes',
    statusUpcoming: '{n} próximos',
    statusUpdated: 'act. hace {m} min',
    statusUpdatedNow: 'ahora mismo',
    dataVintageNow: 'datos frescos',
    dataVintageMinutes: 'datos de hace {m} min',
    dataVintageHours: 'datos de hace {h} h',
    dataVintageDays: 'datos de hace {d} d',
    analystSwitch: 'Analista',
    aboutModel: 'Acerca del modelo',
    drawerHow: 'Cómo funciona SciKick',
    drawerB1: 'Cada equipo tiene dos números: cuánto marca y cuánto encaja. El factor local va incluido.',
    drawerB2: 'De ahí contamos triunfos, empates y derrotas. "Gana 7 de cada 10" es ese conteo, en palabras.',
    drawerCalib: 'Modelo calibrado',
    languageName: 'Idioma',
    matchContext: 'Contexto del partido',
    matchMeta: 'Detalles',
    sectionOverview: 'Resumen',
    sectionMarkets: 'Mercados',
    sectionScorers: 'Goleadores',
    tabContext: 'Contexto',
    tabValue: 'Valor',
    dismissPick: 'Descartar el pick de hoy',
    seeWhy: 'Ver por qué',
  },
} as const

export type TranslationKey = keyof typeof DICTIONARIES['en']
type Dictionary = (typeof DICTIONARIES)['en']

const MARKET_CATEGORY_KEY_TO_I18N: Record<MarketCategoryKey, TranslationKey> = {
  results: 'marketResults',
  goals: 'marketGoals',
  handicap: 'marketHandicap',
  corners: 'marketCorners',
  cards: 'marketCards',
  firstHalf: 'marketFirstHalf',
  halfFull: 'marketHalfFull',
  combined: 'marketCombined',
}

export function marketCategoryLabel(category: MarketCategoryKey): TranslationKey {
  return MARKET_CATEGORY_KEY_TO_I18N[category]
}

const STORAGE_KEY = 'scikick.locale'

function detectInitialLocale(): Locale {
  try {
    const saved = localStorage.getItem(STORAGE_KEY)
    if (saved === 'en' || saved === 'es') return saved
  } catch {
    // Private mode: fall through to browser language
  }
  try {
    return navigator.language.toLowerCase().startsWith('es') ? 'es' : 'en'
  } catch {
    return 'en'
  }
}

interface LanguageContextValue {
  locale: Locale
  setLocale: (locale: Locale) => void
  t: (key: TranslationKey) => string
}

const LanguageContext = createContext<LanguageContextValue | null>(null)

export function LanguageProvider({ children }: { children: ReactNode }) {
  const [locale, setLocale] = useState<Locale>(detectInitialLocale)

  useEffect(() => {
    document.documentElement.lang = locale
    try {
      localStorage.setItem(STORAGE_KEY, locale)
    } catch {
      // Private mode: locale lasts for the session
    }
  }, [locale])

  const t = (key: TranslationKey): string => DICTIONARIES[locale][key as keyof Dictionary]

  return (
    <LanguageContext.Provider value={{ locale, setLocale, t }}>
      {children}
    </LanguageContext.Provider>
  )
}

export function useLanguage(): LanguageContextValue {
  const ctx = useContext(LanguageContext)
  if (!ctx) throw new Error('useLanguage must be used within a LanguageProvider')
  return ctx
}

export function fillVars(template: string, vars: Record<string, string | number>): string {
  let out = template
  for (const [k, v] of Object.entries(vars)) out = out.split(`{${k}}`).join(String(v))
  return out
}
